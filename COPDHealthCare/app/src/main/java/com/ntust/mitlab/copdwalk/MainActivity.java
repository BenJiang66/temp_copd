...
public class MainActivity extends AppCompatActivity
    ...
    private int readStepGoal_using = 0;
    ...
    private String deviceWatch, deviceSPO2, deviceEnv;
...
//                if(getListAverage(hrs)<90)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        stepGoal = Integer.parseInt(MyShared.getData(this, "StepGoal"));
        ...
        public void onClick(View view) {
            switch (view.getId()){
                ...
                case R.id.btnGoAct:
                    ...
                    //dbHelper.deleteDaily("1540051200000");//刪除剛剛加入的今天資料
                    /*
                    //save daily
                    long Mills_In_Days = 24*60*60*1000;
                    final DBHelper dbHelper = DBHelper.getInstance(MainActivity.this);
                    dbHelper.deleteDaily("1540051200000");//刪除剛剛加入的今天資料
                    //dbHelper.deleteMassData();
                    //dbHelper.addMassData(300);

                    Long today = Util.getToday();
                    int steps = dbHelper.getSteps(today, today+Mills_In_Days);
                    int h_i_time = dbHelper.getHItime(today.longValue(), today+Mills_In_Days);
                    //步數為0就不要新增
                    if(steps==0)
                        return;
                    boolean state = dbHelper.saveDaily(today, steps, h_i_time, (int)(steps*0.35), 0);
                    Log.d("MMMMM1","新增昨天的每日步數:"+(state?"成功":"失敗"));
                    */
                    //sync daily
                    /*
                    ...
                    Log.d("MMMMM2","getUnsyncDaily data: "+data);
                    try {
                        ...
                        for(int i=0;i<unsync.length();i++){
                            ...
                            String id = MyShared.getData(MainActivity.this,"id");
...
//                    Toast.makeText(this,"未取得權限",Toast.LENGTH_LONG).show();
    ...
    protected void onResume() {
        ...
        deviceEnv=MyShared.getData(MainActivity.this,DEVICE_TYPE.ENV.toString());
        DBHelper dbHelper = DBHelper.getInstance(this);
    ...
    private void readStepGoal() {
        readStepGoal_using++;//only one readStepGoal() can show AlertDialog at the same time
        final Calendar c = Calendar.getInstance();
        c.setTimeZone(TimeZone.getTimeZone("GMT+8:00"));
        String day_of_week = String.valueOf(c.get((Calendar.DAY_OF_WEEK)));
        Log.d("day od week", day_of_week);
        String date_checked = MyShared.getData(this, "StepGoal_check_date");
        Log.d("--test---", date_checked);
        c.setTimeInMillis(Long.parseLong(date_checked));
        final Date last_check_day = c.getTime();

        if(Util.getToday() > last_check_day.getTime() && readStepGoal_using==1)
        {
            //do the steps_week check
            if(steps_week/7 > stepGoal)
            {
                AlertDialog ad = new AlertDialog.Builder(MainActivity.this)
                        .setTitle("已達成本周目標步數!")
                        .setMessage("本周目標步數將提升500步")
                        .setPositiveButton("好", new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                int stepGoal_fix = stepGoal + 500;
                                MyShared.setData(MainActivity.this, "StepGoal", String.valueOf(stepGoal_fix));
                                stepGoal = stepGoal_fix;
                                //setting next check date (from today's next monday)
                                c.setTimeInMillis(Util.getToday());
                                int today_day_of_week = c.get(Calendar.DAY_OF_WEEK);
                                int offset_check = 9 - today_day_of_week;
                                Date next_check_day = new Date(Util.getToday() + (long)offset_check*24*60*60*1000);
                                MyShared.setData(MainActivity.this, "StepGoal_check_date", String.valueOf(next_check_day.getTime()));
                                readStepGoal_using--;
                            }
                        })
                        .setCancelable(false)
            ...
            else
            {
                AlertDialog ad = new AlertDialog.Builder(MainActivity.this)
                        .setTitle("未達成本周目標步數!")
                        .setMessage("請選擇：")
                        .setPositiveButton("減少500步", new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                int stepGoal_fix = stepGoal - 500;
                                MyShared.setData(MainActivity.this, "StepGoal", String.valueOf(stepGoal_fix));
                        ...
                        .setNegativeButton("維持步數", new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                //Did nothing, but need to do questionnaire
                                //setting next check date (from today's next monday)
                                c.setTimeInMillis(Util.getToday());
                                int today_day_of_week = c.get(Calendar.DAY_OF_WEEK);
                                int offset_check = 9 - today_day_of_week;
                        ...
                        .show();
                setDialogParam(ad);
                //show alertdoalot 1.decrease stepGoal and save 2.retain stepGoal
                // then fill some questionnaire?
            }
        }
    ...
    private final BroadcastReceiver broadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothLeService.ACTION_GATT_CONNECTED.equals(action))
            {
                if(type.equals(DEVICE_TYPE.WATCH.toString())){
                ...
                }else if(type.equals(DEVICE_TYPE.SPO2.toString())){
                    ...
                    final String deviceAddr_ENV = MyShared.getData(MainActivity.this, DEVICE_TYPE.ENV.toString());
                    //使用AsyncTask避免Toast一起被sleep
                    new AsyncTask<Void, Void, Void>(){
                        @Override
                        protected void onPreExecute() {
                            //super.onPreExecute();
                            Toast.makeText(MainActivity.this,"成功連線到環境盒子",Toast.LENGTH_SHORT).show();
                        }
                        @Override
                        ...
                        protected void onPostExecute(Void aVoid) {
                            ...
                            mBluetoothLeService.sendData2(deviceAddr_ENV,"b" );
                        }
                    }.execute();
                }
            ...
            else if (BluetoothLeService.ACTION_GATT_DISCONNECTED.equals(action))
            {
                ...
                if(type.equals(DEVICE_TYPE.WATCH.toString())){
                    ...
                    Toast.makeText(MainActivity.this,"環境盒子斷線",Toast.LENGTH_SHORT).show();
                }
...
//        }
    ...
    private void t()
    {
        //int p = ActivityCompat.checkSelfPermission(this, Manifest.permission.OVERRIDE_WIFI_CONFIG);
    }
    private void updateEnvData(String temperature, String humidity, String pm, String uv){
        ...
        HttpTask httptask = new HttpTask("POST", json, "/env/add", null);
        Log.d("/env/add","json "+json);
        httptask.setCallback(new AsyncResponse() {
            @Override
            public void processFinish(int state, String result, String endPoint) {
                if(state==200)
