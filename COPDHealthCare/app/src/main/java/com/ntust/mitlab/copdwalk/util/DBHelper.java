...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public final static String TABLE_STEPGOAL = "stepgoal";
...
//        return jsonArray.toString();
    ...
    //儲存每日的步數，每天一筆
    public boolean saveDaily(long date, int step, int h_i_time, int distance, int sync_flag){
        boolean flag = false;
    ...
    public int getStepGoal() {
        ...
        if(c.getCount() == 0)
            ...
            c.moveToFirst();
            step_goal = c.getInt(0);
        }

        return step_goal;
    }
    public void updateStepGoal(int StepGoal_ori, int StepGoal_fix) {
        ContentValues cv = new ContentValues();
        cv.put("step_goal", String.valueOf(StepGoal_fix));
        instance.getWritableDatabase().update(TABLE_STEPGOAL, cv, "step_goal="+StepGoal_ori, null);
    }
    public boolean getFlagCheck(){
        return false;
    }
    public void updateFlagCheck(boolean FlagCheck_ori, boolean FlagCheck_fix){

    }
    public long getStepGoalDB(String ColName){
        int result = 0;
        Cursor c = getReadableDatabase().query(
                TABLE_STEPGOAL,
                new String[]{ColName},
    ...
    public void updateStepGoalDB(String ColName, Long Data_ori, Long Data_fix)
    {
        ContentValues cv = new ContentValues();
        cv.put("step_goal", String.valueOf(Data_fix));
        instance.getWritableDatabase().update(TABLE_STEPGOAL, cv, ColName+"="+Data_ori, null);
    }
    private void check_stepDB_hasData() {
        Cursor c = getReadableDatabase().query(
                TABLE_STEPGOAL,
                null,
                null,
        ...
        if(c.getCount() == 0)
        {
            ...
            cv.put("step_goal", 0);
            cv.put("flag_check", 0);
            getWritableDatabase().insert(TABLE_STEPGOAL, null, cv);
        }
    }
    */
    public void deleteDaily(String date){
        getReadableDatabase().execSQL("DELETE FROM daily WHERE date = "+ date);
    }
    public void addMassData(int n){
        long date = 1540126800000L;
        for(int i = 0; i<n; i++) {
            long date_i = date + i;
            getReadableDatabase().execSQL("INSERT INTO pdo (date, steps) VALUES (" + date_i + ", 100)");
        }
    }
    public void deleteMassData(){
        getReadableDatabase().execSQL("DELETE FROM pdo WHERE date > 1540126800000");
    }
    public void delete(){
        getReadableDatabase().execSQL("DELETE FROM pdo WHERE date = 1540040400100");
    }
}
